    ...
    let mut text = match app.current_view() {
        View::Main => {
            ...
            if !app.db_names.is_empty() {
                hints.push(format!("{}/{}: entries", key_label(&cfg.keybindings.up), key_label(&cfg.keybindings.down)));
        ...
        View::Preview => {
            "Esc: close preview • q: quit".to_string()
        }
        View::CreateEntry => {
            "Tab: switch fields • Enter: create • Esc: cancel".to_string()
        }
        View::EditEntry => {
            "Tab: switch fields • Enter: save • Esc: cancel".to_string()
        }
        View::DeleteConfirm => {
            "Y/Enter: confirm delete • N/Esc: cancel".to_string()
        }
    };
