...
use crate::app::App;
use crate::util::{format_kv_entry, truncate_with_ellipsis, format_size};
...
pub fn render(f: &mut Frame, area: Rect, app: &App) {
    ...
    let items: Vec<ListItem> = app.db_names
        .iter()
        .enumerate()
        .map(|(i, name)| {
    ...
    let kv_items: Vec<ListItem> = app.entries
        .iter()
        .enumerate()
        .map(|(i, (k, v))| {
    ...
    let kv_list = List::new(kv_items).block(
        Block::default().borders(Borders::ALL).title(entry_title)
    );
...
fn render_stats_panel(f: &mut Frame, area: Rect, app: &App) {
    ...
    let bookmarked_in_current_db = if let Some(db_name) = app.db_names.get(app.selected) {
        app.entries.iter().filter(|(k, _)| app.bookmarks.contains(db_name, k)).count()
    } else {
        0
    };
    ...
    if !app.entries.is_empty() {
        ...
        let avg_key_len: f64 = app.entries.iter().map(|(k, _)| k.len()).sum::<usize>() as f64 / app.entries.len() as f64;
        let avg_val_len: f64 = app.entries.iter().map(|(_, v)| v.len()).sum::<usize>() as f64 / app.entries.len() as f64;
