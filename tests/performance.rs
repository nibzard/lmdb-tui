use heed::types::Str;
use lmdb_tui::db::{
    env::{count_entries, list_entries_paginated, open_env},
    query::{self, count_matches, scan_paginated, Mode},
};
...
fn test_memory_efficiency_pagination() -> anyhow::Result<()> {
    ...
    for (i, (key, _)) in small_page.iter().enumerate() {
        let expected = format!("key_{:06}", i);
        assert_eq!(*key, expected);
    }
...
fn test_performance_regression() -> anyhow::Result<()> {
    ...
    println!(
        "Count duration: {:?}, Scan duration: {:?}",
        count_duration, scan_duration
    );
    ...
    assert!(
        count_duration <= scan_duration * 2,
