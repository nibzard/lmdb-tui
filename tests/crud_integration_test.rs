use anyhow::Result;
use lmdb_tui::app::{Action, App, DialogField, View};
...
fn test_create_entry_dialog_flow() -> Result<()> {
    ...
    app.reduce(Action::ExecuteCommand(
        lmdb_tui::app::CommandId::CreateEntry,
    ))?;
    ...
    let found = app
        .entries
        .iter()
        .any(|(k, v)| k == "test_key" && String::from_utf8_lossy(v) == "test_value");
...
fn test_edit_entry_dialog_flow() -> Result<()> {
    ...
    let found = app
        .entries
        .iter()
        .any(|(k, v)| k == "original_key" && String::from_utf8_lossy(v) == "modified_value");
...
fn test_delete_entry_dialog_flow() -> Result<()> {
    ...
    app.reduce(Action::ExecuteCommand(
        lmdb_tui::app::CommandId::DeleteEntry,
    ))?;
...
fn test_dialog_cancellation() -> Result<()> {
    ...
    assert_eq!(
        app.entries.len(),
        initial_count,
        "No entries should be added on cancel"
    );
    ...
    if !app.entries.is_empty() {
        ...
        assert_eq!(
            app.entries.len(),
            initial_count,
            "No entries should be deleted on cancel"
        );
    }
